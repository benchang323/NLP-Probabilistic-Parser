...
class EarleyChart:
    ...
    def _run_earley(self) -> None:
        ...
        self.cols = [Agenda(grammar=self.grammar) for _ in range(len(self.tokens) + 1)]
        ...
        for i, column in tqdm.tqdm(enumerate(self.cols),
            ...
            while column:    # while agenda isn't empty
                ...
                if next is None:
                    ...
                    # Speedup: Batch duplicate check
                    if column.predicted.__contains__(next):
                        pass
                    else:
                        self._predict(next, i)
                        column.predicted.add(next)
                else:
    ...
    #                 new_item = customer.with_dot_advanced().add_weight(item.weight)
    #                 # Adding both the completed item and the customer as backpointers to the new item
    #                 new_item.add_backpointer(item)
    #                 new_item.add_backpointer(customer)
    #                 self.cols[position].push(new_item)
    #                 log.debug(f"\tAttached to get: {new_item} in column {position}")
    #                 self.profile["ATTACH"] += 1

    # Speedup: Index Customer
            nonterminal_wanted = item.rule.lhs
            for customer_index in self.cols[mid]._wanted_index[nonterminal_wanted]:
                customer = self.cols[mid]._items[customer_index]
                if customer.next_symbol() == item.rule.lhs:
                    new_item = customer.with_dot_advanced().add_weight(item.weight)
                    new_item.add_backpointer(item)
                    new_item.add_backpointer(customer)
...
class Agenda:
    ...
    def __init__(self, grammar) -> None:
        ...
        self._best_weight: Dict[Item, float] = {}
        self._wanted_index: Dict[str, List[int]] = {}   #Speedup: Index customer
        self.predicted = set()   #Speedup: Batch duplicate check
...
#             self._best_weight[item] = item.weight
#         if item in self._index.keys() and item.next_symbol() is None:
#             if item.weight < self._best_weight[item]:
#                 self._items.pop(self._index[item])
#                 self._items.append(item)
#                 self._index[item] = len(self._items) - 1               
#                 self._best_weight[item] = item.weight
        
        if len(self._best_weight) == 0:
            self._items.append(item)
            self._index[item] = len(self._items) - 1
            self._best_weight[item] = item.weight
        else:
            duplicate = False
            duplicate_item = None
            for key in self._best_weight:
                if item.rule.__repr__ == key.rule.__repr__ and item.start_position == key.start_position and item.dot_position == key.dot_position:
                    duplicate = True
                    duplicate_item = key
                    break
            if not duplicate:
                ...
                if item.dot_position > 0 and item.weight < self._best_weight[duplicate_item]:
                    if self._index[duplicate_item] >= self._next:
                        self._items.remove(duplicate_item)
                    del self._best_weight[duplicate_item]
                    
                    self._items.append(item)
                    self._index[item] = len(self._items) - 1               
                    self._best_weight[item] = item.weight

#             if item not in self._best_weight:
#                 self._items.append(item)
#                 self._index[item] = len(self._items) - 1  
#                 self._best_weight[item] = item.weight
#             if item in self._best_weight and item.weight < self._best_weight[item]:
#                 if self._index[item] >= self.next:
#                     self._items.pop(self._index[item])
#                     self._items.append(item)
#                     self._index[item] = len(self.items) - 1
#                     self._best_weight[item] = item.weight
    ...
    def pop(self) -> Item:
        ...
        current_index = self._next
        self._next += 1
        
        # Speedup: Index customer
        if self.grammar.is_nonterminal(item.next_symbol()):
            nonterminal_wanted = item.next_symbol()
            if nonterminal_wanted not in self._wanted_index:
                self._wanted_index[nonterminal_wanted] = [current_index]
            else:
                self._wanted_index[nonterminal_wanted].append(current_index)        
        return item
...
class Item:
    ...
    start_position: int
    weight: float = 0.0
    backpointers: List[Optional['Item']] = field(default_factory=list, compare=False, hash=False)
    ...
    def add_backpointer(self, item: 'Item') -> None:
        """Associates a given item as a backpointer."""
        # Even though our class is frozen, we can still mutate lists inside it.
        self.backpointers.append(item)
        
    def add_weight(self, increment: float) -> Item:
        return Item(rule=self.rule, dot_position=self.dot_position, start_position=self.start_position, weight=self.weight + increment)
...
#         if self.__repr__ == item.__repr__:
#             return True
#         else:
#             return False
    ...
    def __repr__(self):
        ...
        if self.value is None:
            ...
            for child in self.children:
                sent += ' ' + child.__repr__()
            sent = sent.strip()
        elif not self.grammar.is_nonterminal(self.value):
            sent = ''
            for child in self.children:
                sent += ' ' + child.__repr__()
            sent += ' ' + self.value
...
def build_tree(root, grammar):

    if not root.backpointers:
        if grammar.is_nonterminal(root.rule.rhs[0]):
            return None
        else:
            child = TreeNode(root.rule.rhs[0], grammar)
            return child
    
    if root.next_symbol() is None:
        node = TreeNode(root.rule.lhs, grammar)
    else:
        if grammar.is_nonterminal(root.next_symbol()):
            node = TreeNode(None, grammar)
        else:
            node = TreeNode(root.next_symbol(), grammar)
    
    for item in reversed(root.backpointers):
        child = None
        child = build_tree(item, grammar)    
...
# Speedup: Vocabulary specialization
def specialize_grammar(grammar, tokens) -> Grammar:
    specialized_grammar = copy.deepcopy(grammar)
    unused_keys = []
    for key in specialized_grammar._expansions.keys():
        unused_rules = []
        for rule in specialized_grammar._expansions[key]:
            for symbol in rule.rhs:
                if not specialized_grammar.is_nonterminal(symbol) and symbol not in tokens:
                    unused_rules.append(rule)
                    break
        for unused_rule in unused_rules:
            specialized_grammar._expansions[key].remove(unused_rule) 
        if len(specialized_grammar._expansions[key]) == 0:
            unused_keys.append(key)
    for key in unused_keys:        
        del specialized_grammar._expansions[key]
    ...
    with open(args.sentences) as f:
        for sentence in f.readlines():
            ...
            if sentence != "":  # skip blank lines
                ...
                #new_grammar = specialize_grammar(grammar, sentence.split())
                ...
                if chart.accepted():
                    complete_roots = [item for item in chart.cols[-1].all() if item.rule.lhs == grammar.start_symbol and item.next_symbol() is None and item.start_position == 0]
                    
#                     print(complete_roots)
                    best_root = None
                    best_weight = math.inf
                    for root in complete_roots:
                        if root.weight < best_weight:
                            best_root = root
                            best_weight = root.weight
                                
                    tree = build_tree(best_root, grammar)
                    print(tree)
                    print(best_root.weight)
                else:
                     print("NONE")
